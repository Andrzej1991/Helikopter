...
public class GamePanel extends SurfaceView implements SurfaceHolder.Callback
{
...

    {
        if(player.getPlaying()) {
            ...
            if(!reset)
            {
                ...
                reset = true;
                dissapear = true;
                explosion = new Explosion(BitmapFactory.decodeResource(getResources(),R.drawable.explosion),player.getX(),player.getY()-10,
                        100,100,25);
            ...
            if(resetElapsed>100 &&!newGameCreated)
            {
                newGame();
    ...
    public void newGame()
    {
        ...
        for(int i = 0; i*20<WIDTH+40; i++)
        {
            ...
            {
                botborder.add(new BotBorder(BitmapFactory.decodeResource(getResources(),R.drawable.brick)
                        ,i*25,HEIGHT - minBorderHeight));
            }
            //adding borders until the initial screen is filed
            else
            {
                botborder.add(new BotBorder(BitmapFactory.decodeResource(getResources(), R.drawable.brick),
                        i * 25, botborder.get(i - 1).getY() - 1));
            }
        }
    ...
    public void drawText(Canvas canvas)
    {
        ...
        canvas.drawText("Dystans: " + (player.getScore() * 3), 10, HEIGHT - 10, paint);
        canvas.drawText("Wynik: "+ best, WIDTH - 215, HEIGHT -10, paint );
        ...
        if(!player.getPlaying()&&newGameCreated&&reset)
        {
            ...
            canvas.drawText("Nacisnij by grać: ", WIDTH / 2 - 50, HEIGHT / 2, paint1);

            paint1.setTextSize(20);
            canvas.drawText("Nacisnij i przytrzymaj, żeby lecieć do góry", WIDTH / 2 - 50, HEIGHT / 2 + 20, paint1);
            canvas.drawText("Puść żeby opaść", WIDTH/2-50,HEIGHT/2+40,paint1);
        }
    }
